...
Hyprland compositor config split into modular files. These are Lua
(`hl.*` API), not hyprlang: hyprlang has been deprecated since Hyprland 0.55 and
is dropped in 0.57 — see [the announcement](https://hypr.land/news/26_lua/).

- `hyprland.lua` — monitors, env, visuals, layout, window/layer rules
- `components/autostart.lua` — startup services
- `components/binds.lua` — keybindings
- `components/input.lua` — keyboard/mouse/gesture settings
- `components/programs.lua` — shared `terminal`/`fileManager`/`menu` names
  (hyprlang's `$terminal` globals crossed file boundaries; Lua locals do not)
- `touchpad.lua` — touchpad device settings (scroll/sensitivity, plus the
  `enabled = true` boot default that `apply-touchpad.sh` overrides at runtime)

`exec-once` and `exec` map onto Lua structurally: `exec-once` becomes a
`hl.on("hyprland.start", ...)` handler, while `exec` becomes a top-level
`hl.exec_cmd(...)` — the config file is re-executed on every reload, so those
run at login *and* on each reload, exactly as before. Type stubs for the whole
API ship at `/usr/share/hypr/stubs/hl.meta.lua`, and `Hyprland --verify-config
-c <file>` checks a config without running it.
...
- **Cheatsheet** — Full-screen keybinding overlay (`Super + /`). Per-app tabs
  ...
  Hyprland map is generated by `hypr/scripts/gen-keymap.sh`, which runs from
  `autostart.lua` on every reload so the sheet can't drift from the real binds.
  Each bind carries a `desc = "<Category>: <Text>"` in `components/binds.lua`;
  Hyprland exposes it as `.description` in `hyprctl binds -j`, and the generator
  reads *that* rather than scraping the config text, so the compositor is the
  single source of truth. (The script refuses to overwrite the map when the
  compositor reports no descriptions at all, so a session running some other
  config can't degrade the committed file.) It is deliberately ordered *above* the
...
**Edit templates, never outputs.** Every `output_path` in `matugen/config.toml` is
regenerated on each wallpaper change and is gitignored, so edits to the generated file
are silently overwritten within one cycle:

| Template (tracked, edit this) | Output (generated, ignored) |
| --- | --- |
| `matugen/templates/quickshell-colors.json` | `quickshell/theme/colors.json` |
| `matugen/templates/tmux-colors.conf` | `tmux/colors.conf` |
| `matugen/templates/kitty-colors.conf` | `kitty/theme.conf` |
| `matugen/templates/yazi-theme.toml` | `yazi/theme.toml` |
| `matugen/templates/hyprlock.conf` | `hypr/hyprlock.conf` |
| `matugen/templates/gtk-colors.css` | `gtk-3.0/gtk.css`, `gtk-4.0/gtk.css` |
...
**Persistence** is the top-level `hl.exec_cmd` line in `autostart.lua` (not a
`hyprland.start` handler): it runs at login *and* on every config reload.
`touchpad.lua` always declares `enabled = true` as the boot default, so without
that line `Super + Shift + R` would silently re-enable a touchpad you had turned
off. The applier drives the compositor with `hyprctl eval 'hl.device{...}'`,
the Lua-config equivalent of the old `hyprctl keyword device[...]` call.
...
- **awww** — desktop background on all outputs, animated GIFs included (`awww-daemon`, started from `hypr/components/autostart.lua`)
...
> **Do not add `--format bgr`/`rgb` to `awww-daemon`.** The 3-channel formats really are 3/4 the memory, but this compositor rejects them and the daemon core-dumps on startup, leaving no wallpaper at all. See `hypr/components/autostart.lua`.
